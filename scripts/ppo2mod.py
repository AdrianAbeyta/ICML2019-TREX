...
class PPO2(ActorCriticRLModel):
    ...
    def __init__(self, policy, env, gamma=0.99, n_steps=128, ent_coef=0.01, learning_rate=2.5e-4, vf_coef=0.5,
                 ...
                 full_tensorboard_log=False, seed=None, n_cpu_tf_sess=None,learned_reward=False, reward_ckpt_dir=None):
        ...
        self.reward_ckpt_dir=reward_ckpt_dir
    ...
    def _make_runner(self):
        return Runner(env=self.env, model=self, n_steps=self.n_steps,
                      gamma=self.gamma, lam=self.lam, learned_reward=self.learned_reward, reward_ckpt_dir=self.reward_ckpt_dir )
    ...
    def learn(self, total_timesteps, callback=None, log_interval=1, tb_log_name="PPO2",
              reset_num_timesteps=True,learned_reward=False, reward_ckpt_dir=None):
...
class Runner(AbstractEnvRunner):
    def __init__(self, *, env, model, n_steps, gamma, lam, learned_reward=False, reward_ckpt_dir=None):
        """
        ...
        if self.learned_reward == True:
            ...
            saver = tf.train.import_meta_graph('/home/adrian/turtlebot_ws/src/openai_examples_projects/my_turtlebot3_openai_example/log/turtlebot3/max60/gt_traj/model_4.ckpt.meta')
    ...
    def _run(self):
        ...
        for _ in range(self.n_steps):
            ...
            # Preprocess state to feed into reward network - Mask score, convert to float and normalise
                ...
                #rewards= np.copy(reward)
                # state_in[:, 0:10, :, :] = 0
                # state_in = state_in.astype(np.float32, copy=False)
                # state_in /= 255.0
                print(reward)
                
                #rewards = self.sess.run(state_in)           

...
# New additions for openai_ros environment
...
if __name__ == "__main__":
    ...
    # Import arguments from ROS parameter server
    args.env_id = rospy.get_param('task_and_robot_environment_name')
    args.env_type = rospy.get_param('env_type','openai_ros')
    args.learners_path = rospy.get_param('learners_path')
    args.max_chkpt = rospy.get_param('max_chkpt',240)
    args.steps = rospy.get_param('steps',)
    args.max_steps = rospy.get_param('max_steps')
    args.traj_noise = rospy.get_param('traj_noise')
    args.min_length = rospy.get_param('min_length',1000)
    args.num_layers = rospy.get_param('num_layers',2)
    args.embedding_dims = rospy.get_param('embedding_dims',256)
    args.num_models = rospy.get_param('num_models',3)
    args.l2_reg = rospy.get_param('l2_reg',.01)
    args.noise = rospy.get_param('noise',.1)
    args.D = rospy.get_param('D',1000)
    args.N = rospy.get_param('N',10)
    args.log_dir = rospy.get_param('log_dir')
    args.preference_type = rospy.get_param('preference_type')
    args.min_margin = rospy.get_param('min_margin',1)
    args.include_action = rospy.get_param('include_action')
    args.stochastic = rospy.get_param('stochastic')
    args.random_agent = rospy.get_param('random_agent',False)
    args.eval = rospy.get_param('eval')

    # Args for PPO
    args.rl_runs = rospy.get_param('rl_runs',1)
    args.ppo_log_path = rospy.get_param('ppo_log_path','ppo2')
    args.custom_reward = rospy.get_param('custom_reward')
    args.ctrl_coeff = rospy.get_param('ctrl_coeff',0.0)
    args.alive_bonus = rospy.get_param('alive_bonus',0.0)
    args.gamma = rospy.get_param('gamma', 0.99)
    ...
    if not args.eval :
        ...
        print('Training a preference model with arguments: ')
        print(args)
        train(args)
        ...
        if openai_logdir.exists():
            print('openai_logdir already exists.')
            exit()
